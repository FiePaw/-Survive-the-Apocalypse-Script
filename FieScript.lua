--[[
    FieScript v5.4 - Blutiger (English)
]]
...
-- ========== HUD ScreenGui (separate so it NEVER hides with Insert) ==========
...
hudTitle.Position = UDim2.new(0, 0, 0, 0)
...
local itemNames = {
    ...
    "Screws","Shells","Spatula","Tray", "AC", "Satellite Dish", "Refined Metal", "Watch", "MRE", "TV", "Bucket"
}
-- Items that use pickup-then-use logic (proximity collect)
local proximityCollectItems = {"Bandage", "Medkit"}
-- All auto-use items (shown in list)
local autoUseItemNames = {"Bandage","Medkit","Beans","Bloxiade","Bloxy Cola","Long Ammo","Medium Ammo","Pistol Ammo","Shells"}
...
--[[
    AUTO USE LOGIC:
    - Bandage & Medkit: collect-only logic.
      If the item is NOT already in the player's inventory (Backpack or Character),
      and it is found in DroppedItems within radius, pick it up via remote.
      Do NOT equip or use it — just collect it into inventory.
      If already in inventory, skip entirely (do nothing).
    - All other items: standard backpack equip-and-use logic.
]]
...
local function startAutoUse()
    ...
    autoUseConn=RunService.Heartbeat:Connect(function()
        ...
        for _,name in ipairs(autoUseItemNames) do
            ...
            -- ── Collect-only logic for Bandage and Medkit ──
            if table.find(proximityCollectItems, name) then
                -- If already in inventory (backpack or equipped), skip — do NOT use it
                local alreadyHave = backpack:FindFirstChild(name) or char:FindFirstChild(name)
                if alreadyHave then continue end

                -- Not in inventory — look for it in the world and collect it
                if droppedItemsFolder and root then
                    for _,item in ipairs(droppedItemsFolder:GetChildren()) do
                        if item.Name == name then
                            local mp = item.PrimaryPart or getItemMainPart(item)
                            if mp and (mp.Position - root.Position).Magnitude <= radius then
                                -- Only collect (pickup), do not equip or use
                ...
                -- ── Standard backpack equip-and-use logic for other items ──
                ...
                if tool then
                    ...
                    if ar and ar:IsA("RemoteEvent") then ar:FireServer()
                    elseif useItemRemote then useItemRemote:FireServer(tool) end
                    task.wait(0.1)
                end

                -- Also try from world dropped items within radius (non-proximity items)
                if droppedItemsFolder and root then
                    for _,item in ipairs(droppedItemsFolder:GetChildren()) do
                        if item.Name == name then
                            local mp = item.PrimaryPart or getItemMainPart(item)
                            if mp and (mp.Position - root.Position).Magnitude <= radius then
                                if useItemRemote then useItemRemote:FireServer(item)
                                elseif pickUpItemRemote then pickUpItemRemote:FireServer(item) end
                                task.wait(0.1)
                            end
                        end
...
for i, preset in ipairs(presetColors) do
    ...
    tip.Size=UDim2.new(0,80,0,18) tip.Position=UDim2.new(0,5+col*(swatchSize+6),0,swatchY+row*(swatchSize+6)-18)
    ...
    swatch.MouseButton1Click:Connect(function()
        ...
        StatusText.Text = "Theme changed to: "..preset.name
    end)
end
...
mkInfo("Version: 5.4",Color3.fromRGB(255,255,255),16,yOff) yOff=yOff+30
...
mkInfo("  Bandage & Medkit = collect nearby only (skip if owned)",Color3.fromRGB(160,220,255),12,yOff) yOff=yOff+20
...
print("FieScript v5.4 loaded. [Insert] GUI | [Q] Speed | [X] Pickup | [C] AutoUse")